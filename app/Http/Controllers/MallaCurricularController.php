...
class MallaCurricularController extends Controller
{
	...
	public $Privilegio_modulo = 'Malla Curricular';
	public $paginate = 20;
	...
	public function index($id = NULL)
	{
		...
		if ($privilegio->mas_read == 0){
			...
			if ($curso != -1){
				...
				$cantidad = asignatura::where('asignaturas.asg_nivel', '=', $cursos->cur_numero)
	...
	public function create($id)
	{
		...
		if ($privilegio->mas_add == 0){
			...
			$curso = Curso::join('niveles', 'niveles.niv_codigo', '=', 'cursos.niv_codigo')
							...
							->select('cursos.cur_codigo', DB::raw('CONCAT(cursos.cur_numero, "&deg;", cursos.cur_letra, " ", niveles.niv_nombre, " - Profesor Jefe: ", personas.per_nombre, " ", personas.per_apellido_paterno) as name'), 'cursos.cur_numero')
							->first();
			$tabla = MallaCurricularController::arreglo();
			$entidad = array('Nombre' => $this->Privilegio_modulo, 'controller' => 'malla_curricular', 'pk' => 'asg_codigo', 'clase' => 'container col-sm-6 col-sm-offset-3', 'label' => 'container col-md-4');
	...
	public function update($id)
	{
		...
		$asignatura = asignatura::where('asignaturas.asg_nombre', '=', $input['asg_nombre'])->first();
		
		$asign_profe_curso = new asign_profe_curso();
		$asign_profe_curso = asign_profe_curso::find($id);
		$asign_profe_curso->pro_codigo = $profesor->pro_codigo;
		$asign_profe_curso->asg_codigo = $asignatura->asg_codigo;
		$asign_profe_curso->cur_codigo = $input['cur_codigo'];
		$asign_profe_curso->save();
